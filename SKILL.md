...
> [!important] **AI 核心规则**
> 本 Schema 的精简版核心规则已提取至 **CLAUDE.md**，AI 每次对话会优先加载。
> - **CLAUDE.md**：死文档保护、写入前确认、可验证操作、命名规范、Frontmatter、会话沉淀流程等 **必须遵守的规则**
> - **SKILL.md**（本文件）：完整架构说明、设计理念、详细流程等 **参考文档**
>
> 详细参考：
...
narrative: analytical | executive | stream  # 叙事风格（可选，推荐）
...
| 目录 | narrative | 用途 | 视觉特征 |
|------|-----------|------|----------|
| Wiki/ | `analytical` | 知识积累 | 冷色调、图表丰富、结构化 |
| Projects/ | `executive` | 工作产出 | 进度条、状态徽章、交付感 |
| Daily/ | `stream` | 个人探索 | 流动感、时间戳、情绪标签 |
...
```markdown
---
mood: curious | tired | excited | focused | ...
---
...
- **检测到目录** → 自动应用对应叙事风格
- **创建新文档** → 自动填充对应 frontmatter 和 narrative
- **更新文档** → 保持既有叙事风格一致
- **用户未指定 narrative** → 根据文件路径智能推断
...
### 三、Write with Intent（写入前确认）

> **写入 Wiki 前先确认意图，不确定就问，不盲目操作。**
>
> 灵感来源：[Karpathy CLAUDE.md 原则 1 — Think Before Coding](https://github.com/forrestchang/andrej-karpathy-skills)

写入或修改 Wiki/ 页面前，确认三件事：

1. **格式确认**：叙事风格是什么？页面骨架用哪个模板？
2. **链接确认**：新创建的 wikilink 用什么路径格式？（遵循现有惯例）
3. **范围确认**：用户要我做什么？涉及哪些页面？

**不确定时 → 问用户。宁可多问一句，不盲目写 10 页。**

**设计理由**：知识管理场景中，AI 最常见的问题是"没搞清楚就动手"——用错格式、用错链接路径、改了不该改的页面。这条规则在操作前设置一道确认关卡，从源头预防问题。

---

### 四、Verifiable Operations（可验证操作）

> **操作后验证结果，验证不通过 = 操作未完成。**
>
> 灵感来源：[Karpathy CLAUDE.md 原则 4 — Goal-Driven Execution](https://github.com/forrestchang/andrej-karpathy-skills)

重要操作后必须验证：

| 操作类型 | 验证方式 |
|----------|---------|
| 创建页面 | frontmatter 完整 + abstract 存在 + 至少 1 个交叉链接 |
| 修改页面 | 目标部分已改，未误伤其他内容 |
| 创建链接 | 路径格式正确、目标文件存在 |
| 批量操作 | 操作后计数，确认结果符合预期 |
| 记录日志 | 日志条目已写入，格式正确 |
...
**设计理由**：与"二、操作完成自检"互补。自检规则只覆盖日志记录，验证规则覆盖所有操作的质量。常见的失败模式是"以为做完了但实际有残留"——如批量修复后仍有个别遗漏。

**为什么没采纳 Karpathy 的另外两条原则**：

| 原则 | 编程场景 | 知识管理场景 | 采纳决策 |
|------|---------|-------------|---------|
| Simplicity First | 最少代码 | ❌ 知识的价值在于丰富度和连接密度 | 未采纳。entity 页面可能确实需要 200 行 |
| Surgical Changes | 不碰无关代码 | ❌ 知识管理需要 AI 主动发现并修复关联问题 | 未采纳。AI 应主动巡检和改善，而非只做指令 |

核心区别：编程场景中 AI 是工具，越克制越好；知识管理场景中 AI 是维护者，越主动越好（但有边界）。原则 1 和 4 正好划出了这个边界。
...
> **详细参考 → tools/tool-page-guide.md**（含完整规则、反模式、审美检查项）
>
> 来源：GitHub README 社区研究（awesome-readme、Best-README-Template、readme-best-practices、Day8/re-frame）

#### 四条核心原则

1. **首屏原则（Above the Fold）** — 页面第一屏只保留标题 + 摘要 + 可选图表，5 秒内可回答"这是什么"
2. **分隔线克制** — `---` 只用于大板块切换，非每节都加
3. **视觉节奏（Visual Rhythm）** — 段落、列表、表格、callout 交替出现，连续 3 个同结构为反模式
4. **认知漏斗（Cognitive Funnel）** — 章节层层推进（是什么 → 为什么 → 细节 → 延伸），非并列重复
